...
const ProductManager = {
    ...
    loadMaterials: async function() {
        try {
            ...
            console.log(`Loaded ${this.materials.length} material categories`);
        } catch (error) {
    ...
    enrichProduct: function(product) {
        ...
        // Get material image from materials database
        let materialImage = null;
        if (material) {
            materialImage = this.getMaterialImage(material);
        }
    ...
    // Get material image from materials database
    getMaterialImage: function(materialName) {
        for (const category of this.materials) {
            for (const type of category.types) {
                if (type.en_name === materialName || type.name === materialName) {
                    return type.image;
                }
            }
        }
        ...
        return `
            <div class="product-card group bg-surface-dark rounded-xl border border-white/5 overflow-hidden hover:shadow-xl hover:border-primary/30 transition-all duration-300 flex flex-col" data-product-id="${product.slug}">
                <div class="relative aspect-square bg-deep-navy overflow-hidden">
                    ...
                    <div class="absolute bottom-3 right-3 w-16 h-16 rounded-full overflow-hidden border-2 border-primary/30 bg-deep-navy/80 flex items-center justify-center">
                        <img class="w-full h-full object-cover" src="${product.materialImage}" alt="${product.material} material">
                    </div>
                    ` : ''}
