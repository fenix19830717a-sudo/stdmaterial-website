const GrindingEquipmentB2B = {
    ...
    init: async function() {
        console.log('Initializing Grinding Equipment B2B...');
        ...
        console.log('Grinding Equipment B2B initialized successfully');
    },

    loadData: async function() {
        try {
            const [products, customers, orders, inquiries] = await Promise.all([
                this.fetchData('products.json'),
                this.fetchData('customers.json'),
                this.fetchData('orders.json'),
                this.fetchData('inquiries.json')
            ]);
            
            this.state.products = products.products || [];
            this.state.customers = customers.customers || [];
            this.state.orders = orders.orders || [];
            this.state.inquiries = inquiries.inquiries || [];
        ...
        try {
            ...
            switch (formType) {
                case 'contact':
                    ...
                    break;
                case 'inquiry':
                    await this.submitInquiryForm(data);
                    break;
                case 'quote':
    ...
    submitInquiryForm: async function(data) {
        const inquiry = {
            id: `INQ-${Date.now()}`,
            ...data,
            status: 'New',
    ...
    submitQuoteRequest: async function(data) {
...
document.addEventListener('DOMContentLoaded', () => {
    GrindingEquipmentB2B.init();
});

window.GrindingEquipmentB2B = GrindingEquipmentB2B;
